  ...
  const fetchCats = async () => {
    ...
    try {
      const favourites = await getFavourites(
        localStorage.getItem("CAT_SUB_ID")
      );
    ...
    >
      ...
      {!cats.length && !catsInflight && (
        <Alert mx="auto" variant="secondary" mb={2}>
          ...
          <Link sx={{ color: "background" }} to="/upload">
            uploading
          </Link>
          &nbsp; some
        </Alert>
          ...
          >
            ...
            <Button
              sx={{
                ...
                right: 2,
                backgroundColor: "secondary",
                color: Object.keys(favourites).includes(cat.id)
                  ? "#ff6868"
                  : "white"
              }}
              variant="secondary"
              type="button"
              onClick={() =>
                Object.keys(favourites).includes(cat.id)
                  ? handleUnfavourite(cat.id)
                  : handleFavourite(cat.id)
              }
            >
